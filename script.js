const state = {
  ...
  sanMoodEnabled: localStorage.getItem("turtleSoupSanMood") !== "off",
};
...
const els = {
  ...
  sanMoodBtn: document.querySelector("#sanMoodBtn"),
...
function showView(view) {
  ...
  els.createRoomPanel.classList.toggle("hidden", state.role !== "host");
...
function renderLobbyRoom() {
  ...
  els.enterGameBtn.textContent = `以${state.user?.name || state.nickname || "当前用户"}进入游戏`;
  renderAdminRooms();
}

function renderAdminRooms() {
  if (state.role !== "admin") return;
  if (!state.adminRooms.length) {
    els.adminRoomList.innerHTML = '<p class="empty-state">暂无可巡查房间。</p>';
    return;
  }

  els.adminRoomList.innerHTML = state.adminRooms
    .map(
      (room) => `
        <article class="admin-room-card">
          ...
          <div class="admin-room-meta">
            ...
            <span>${room.locked ? "已锁定" : "开放"}</span>
            <span>${room.questionCount} 问</span>
            <span>${room.chatCount} 聊天</span>
          </div>
          <button type="button" data-room-code="${escapeHtml(room.code)}">查房</button>
        </article>
      `,
    )
    .join("");
...
function renderMemberCard(member) {
  const canKick = canManageRoom() && member.id !== state.user?.id && !isHostMember(member);
  return `
...
function renderSan() {
  ...
  document.body.classList.toggle("san-mood-off", !state.sanMoodEnabled);
  document.body.classList.toggle("san-warning", state.sanMoodEnabled && ratio <= 0.3 && ratio > 0.2);
  document.body.classList.toggle("san-danger", state.sanMoodEnabled && ratio <= 0.2 && ratio > 0.1);
  document.body.classList.toggle("san-critical", state.sanMoodEnabled && ratio <= 0.1);
  els.sanMoodBtn.textContent = state.sanMoodEnabled ? "SAN 氛围" : "氛围关闭";
  els.sanMoodBtn.setAttribute("aria-pressed", String(state.sanMoodEnabled));
}
...
function renderVoice() {
  ...
  els.voiceList.innerHTML = state.room.members
    .map((member) => {
      ...
      const canBlock = canManageRoom() && member.id !== state.user?.id;
      const canKick = canManageRoom() && member.id !== state.user?.id && !isHostMember(member);
      return `
...
function renderAvatar(member) {
  ...
  return `<img class="avatar" src="${escapeHtml(src)}" alt="${escapeHtml(member.name)}头像" onerror="this.onerror=null;this.src='/assets/avatars/default.svg';" />`;
}
...
els.sanMoodBtn.addEventListener("click", () => {
  state.sanMoodEnabled = !state.sanMoodEnabled;
  localStorage.setItem("turtleSoupSanMood", state.sanMoodEnabled ? "on" : "off");
  renderSan();
});
